...
from aster_core.hdf_utils import parse_meta,get_transform,get_projection
...
def process_tile(tile_index, input_file, global_grid, bands=None):
    ...
    if 'tif' in input_file:
        ...
        data = extract_granule(input_file, bands, tile_bbox, global_grid.tile_size, global_grid.projection)
...
def get_bbox_from_aster(hdf_file, dst_crs='epsg:3857'):
    ...
    geotransform = get_transform(meta_parser, 'ImageData0')
    affine = geotransform_to_affine(geotransform)
    bbox = affine_to_bbox(affine, ds.RasterYSize, ds.RasterXSize)
