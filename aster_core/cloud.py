...
def cal_spectral_info(aster_img,nodata_value=0,mask=None,percent=(0,100)):
    if mask is None:
        mask = aster_img[0]!=nodata_value
    # aster_img[:,mask]=np.nan

    aster_vnir_sum = np.sum(aster_img[[0,1,2]],axis=0)
    low_threashold = np.percentile(aster_vnir_sum[mask],percent[0])
    high_threashold = np.percentile(aster_vnir_sum[mask],percent[1])

    mask = (aster_vnir_sum>=low_threashold) & (aster_vnir_sum<=high_threashold)
    
    spectral_info = np.mean(aster_img[:,mask],axis=-1)
    return spectral_info

def cal_cloud_mask(img,bands=[0,1,2,3],lower_percent=99,band1_threashold=None):

    band1_threashold = max(filters.threshold_otsu(img[0][img[0]>0],nbins=256),0.15)

    cloud_spectral = cal_spectral_info(img,percent=(lower_percent,100))
    spectral_dist = np.sum((cloud_spectral[:,np.newaxis,np.newaxis]-img)[bands],axis=0)
    
    cloud_thresh_otsu = filters.threshold_otsu(spectral_dist[spectral_dist>0],nbins=256)
    cloud_mask = (spectral_dist<cloud_thresh_otsu) & (img[0]>band1_threashold)

    if np.any(cloud_mask):
        cloud_spectral = cal_spectral_info(img,mask=cloud_mask)
        spectral_dist = np.sum((cloud_spectral[:,np.newaxis,np.newaxis]-img)[bands],axis=0)
